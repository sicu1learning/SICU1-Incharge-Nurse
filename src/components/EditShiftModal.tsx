...
import { DEFAULT_NURSES } from './AddShiftModal';
import { findPreviousShiftInList } from '../utils/shiftUtils';
...
interface EditShiftModalProps {
  ...
  onSaveShift: (updatedShift: ShiftInfo) => void;
...
export const EditShiftModal: React.FC<EditShiftModalProps> = ({
  ...
  const [shiftType, setShiftType] = useState<ShiftType>('เวรเช้า');

  const allShifts = React.useMemo(() => {
    const list = shiftsHistory && shiftsHistory.length > 0 ? [...shiftsHistory] : [];
    if (previousShift && !list.some((s) => s.id === previousShift.id)) {
      list.push(previousShift);
    }
    return list;
  }, [shiftsHistory, previousShift]);

  const resolvedPreviousShift = React.useMemo(() => {
    const found = findPreviousShiftInList(allShifts, date, shiftType, shift?.id);
  ...
  // Specialized Shift Modules Data States - เริ่มต้นเป็น 0
  ...
  // Load shift data when modal opens or target shift ID changes
  const wasOpenRef = useRef(false);
  const loadedShiftIdRef = useRef<string | null>(null);

  useEffect(() => {
    if (isOpen && shift && (!wasOpenRef.current || loadedShiftIdRef.current !== shift.id)) {
      wasOpenRef.current = true;
      loadedShiftIdRef.current = shift.id;
      setDate(shift.date || '');
      setShiftType(shift.shiftType);
      setInchargeName(shift.inchargeName || '');
      setNurseError(false);
      ...
      setCarriedOver(stats.carriedOver ?? 0);
      setTransferredIn(stats.transferredIn ?? 0);
      setAdmittedNew(stats.admittedNew ?? 0);
      setTransferredOut(stats.transferredOut ?? 0);
      setAgainstAdvice(stats.againstAdvice ?? 0);
      setDeceased(stats.deceased ?? 0);
      setDeceasedPostOp24Hr(stats.deceasedPostOp24Hr ?? 0);
      setAdmitDischarge24Hr(stats.admitDischarge24Hr ?? 0);
      setReferOut(stats.referOut ?? 0);
      setCategory5(stats.category5Count ?? 0);
      setCategory4(stats.category4Count ?? 0);
      setVentilatorCount(stats.ventilatorCount ?? 0);
      setOxygenCount(stats.oxygenCount ?? 0);
      setPostOpCount(stats.postOpCount ?? 0);

      // Populate specialized module data
      setConsultationData(
        shift.consultationData
          ? {
              totalCount: shift.consultationData.totalCount ?? 0,
              departmentCounts: { ...(shift.consultationData.departmentCounts || {}) },
              customDepartments: shift.consultationData.customDepartments
                ? [...shift.consultationData.customDepartments]
                : [],
              notes: shift.consultationData.notes || '',
              items: shift.consultationData.items ? [...shift.consultationData.items] : [],
            }
          : { totalCount: 0, departmentCounts: {}, customDepartments: [], notes: '', items: [] }
      );
      setStaffData(
        shift.staffData || {
          headCount: 0,
      ...
      // Populate movement records
      if (shift.movementRecords && shift.movementRecords.length > 0) {
        setMovementList(
          shift.movementRecords.map((r, idx) => ({
            id: r.id || `mv-${idx}`,
      ...
      } else if (shift.movementNotes && shift.movementNotes.length > 0) {
        setMovementList(
          shift.movementNotes.map((note, idx) => ({
            id: `legacy-${idx}`,
            patientName: '',
            diagnosis: '',
            status: '',
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const updatedStats: PatientStats = {
    ...
    const validMovementRecords: MovementRecord[] = movementList
      ...
      .map((r, idx) => ({
        id: r.id.startsWith('mv-') || r.id.startsWith('move-') ? r.id : `move-${Date.now()}-${idx}`,
    ...
    const updatedShift: ShiftInfo = {
      ...
      updatedAt: `${date} ${new Date().toLocaleTimeString('th-TH', { hour: '2-digit', minute: '2-digit' })} น. (แก้ไข)`,
      stats: updatedStats,
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-xs p-3 sm:p-4 overflow-y-auto animate-in fade-in">
      <div className="bg-white rounded-3xl shadow-2xl max-w-[620px] w-full overflow-hidden border border-slate-200 my-auto">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-bold text-base text-white font-['Prompt',sans-serif]">
                แก้ไขข้อมูลเวร ({shift.date} · {shift.shiftType})
              </h3>
              <p className="text-xs text-slate-300 mt-0.5">
                ปรับปรุงข้อมูลยอดผู้ป่วย หัวหน้าเวร หรือข้อมูลย้าย/รับ/จำหน่าย
              </p>
            </div>
          </div>
          <button
        ...
        <form onSubmit={handleSubmit} className="p-5 sm:p-6 space-y-4 max-h-[80vh] overflow-y-auto">
          ...
          {/* Row 4: ข้อมูล ย้าย/รับ/จำหน่าย เพิ่มเติม */}
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[11px] font-semibold text-teal-800 bg-teal-50 px-2 py-0.5 rounded-full border border-teal-200">
                  {movementList.filter((m) => m.patientName.trim() || m.diagnosis.trim() || m.status.trim() || m.note).length} รายการ
                </span>
              </div>
              <button
                type="button"
      ...
      <ConsultationModal
        ...
        previousConsultationData={previousConsultationData || resolvedPreviousShift?.consultationData}
